...
|---|--------|-------------------|----------------------------|-------------------|
| 1 | Acceso no autorizado a datos personales (PII) | Tool / Resource | Alta | Un atacante invoca `consultar_cliente(123)` y obtiene nombre, email, dirección, etc. |
| 2 | Abuso de envío de emails (spam / phishing) | Tool | Alta | Automatiza llamadas a `enviar_email(...)` para mandar miles de correos desde el dominio de la empresa. |
| 3 | Exfiltración de credenciales (SMTP / DB) expuestas como resources | Resource | Alta | Lee `config://email` y roba usuario/contraseña SMTP para reutilizarlos fuera del MCP. |
| 4 | MITM y robo de tokens/datos por falta de HTTPS | Tool / Resource / Prompt | Alta | Intercepta tráfico HTTP en red pública y captura respuestas con PII o tokens de autenticación. |
| 5 | Prompt injection / data poisoning desde recursos o inputs | Prompt / Resource | Media | Inserta texto malicioso en campos (p.ej. nombre de cliente) para inducir al LLM a revelar secretos o ejecutar acciones indebidas. |
| 6 | DoS por invocaciones masivas o parámetros costosos | Tool | Media | Realiza miles de llamadas concurrentes a `generar_informe(...)` con rangos de fechas enormes saturando CPU/DB. |
...
Cliente MCP                    Auth Server                    Servidor MCP
     ...
     │  ──────────────────────────► │                              │
     │                              │                              │
    │  2. 200 OK {access_token(JWT), token_type:"Bearer", expires_in} │
     │  ◄────────────────────────── │                              │
     │                              │                              │
    │  3. POST /mcp (o /rpc)  + tool/resource requests                    │
    │     Header: Authorization: Bearer <access_token>                   │
     │  ──────────────────────────────────────────────────────────────────►│
     │                              │                              │
    │                              │  4. Validar JWT (firma, exp, iss, aud, scopes/roles) │
     │                              │  ◄──────────────────────────  │
     │                              │                              │
    │                              │  5. (Opcional) GET /.well-known/jwks.json o introspección del token │
    │                              │  ──────────────────────────► │
     │                              │                              │
    │  6. 200 OK (respuesta MCP) o 401/403 si no autorizado              │
     │  ◄──────────────────────────────────────────────────────────────────│
```
...
```json
{
    "sub": "cliente_app_soporte_01",
    "iss": "https://auth.empresa.example",
    "exp": "<epoch+3600>",
    "iat": "<epoch_actual>",
    "permissions": ["clientes:read", "informes:read"],
    "allowed_tools": ["consultar_cliente", "generar_informe"],
    "aud": "mcp-servidor-clientes"
}
```
...
|-----------|--------|------------------------|
| 1 (Crítica) | Autenticación + autorización (RBAC/ABAC) | JWT Bearer obligatorio en todas las rutas; validar `iss/aud/exp`; mapear `permissions` a tools/resources; denegar por defecto (403) |
| 2 (Crítica) | Cifrado en tránsito | Exponer solo por HTTPS (TLS) detrás de un reverse proxy (Caddy/Nginx); HSTS; deshabilitar HTTP plano |
| 3 (Alta) | Gestión segura de secrets y resources | Eliminar credenciales de `config://email`; usar Vault/Secrets Manager; en resources devolver valores enmascarados o metadatos no sensibles |
| 4 (Media) | Rate limiting + cuotas + protección DoS | Límite por IP/cliente (p.ej. 60 req/min); timeouts; límites de tamaño de payload; límites de rango en `generar_informe` |
| 5 (Media) | Observabilidad y auditoría | Logs estructurados (quién, qué tool/resource, parámetros sensibles redacted, resultado, latencia); alertas; trazabilidad por `request_id` |
...
```
SEGURIDAD:
- Autenticación: OAuth2 Client Credentials → JWT Bearer (tokens de corta duración + rotación de claves)
- Autorización: Mixta (por roles + por herramienta/resource), con política deny-by-default
- Datos sensibles: PII (clientes), credenciales (SMTP/DB), contenido de emails, informes con datos agregados sensibles
- Validación de inputs: Sanitización y validación estricta de parámetros (IDs/rangos), prevención de inyección (SQL/command), límites de tamaño, allowlists de destinos/plantillas de email
- Logging: Autenticación (éxito/fallo), intentos 401/403, tool/resource invocados (con redacción de datos sensibles), cambios/acciones de escritura, errores, latencias y `request_id`

DESPLIEGUE:
- Transporte: Streamable HTTP (detrás de reverse proxy TLS); stdio solo para desarrollo local
- Infraestructura: Docker en VPS o Kubernetes (según carga), con secretos gestionados externamente
- Escalabilidad: Stateless + escalado horizontal (réplicas); colas para trabajos pesados (informes); cache para lecturas frecuentes
- Monitorización: RPS, p95/p99 latencia por tool, tasa de errores, uso CPU/RAM, métricas DB (conexiones/tiempos), tasa 401/403, tasa de tool calls por cliente
- Backup y recuperación: Backups automáticos de DB + pruebas de restore; versionado de configuración; runbooks de incidentes; despliegues reproducibles (IaC)
```
