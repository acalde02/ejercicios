...
|-----------|-------------|---------------|
| A | 0 | Da respuesta concreta y solida | 
| B | 0.7 | Sigue dando una respuesta concreta y solida pero consume mas tokens|
| C | 1.5 | Sigue dando respuesta solida |
...
---
# Análisis de Variación de Temperature

## Documentación por ejecución

### 1. ¿La respuesta es idéntica o diferente entre ejecuciones con el mismo temperature?

- Con `temperature = 0`:  
  La respuesta tiende a ser idéntica o extremadamente similar entre ejecuciones. El modelo elige siempre los tokens con mayor probabilidad, por lo que el resultado es determinístico.

- Con `temperature = 0.7`:  
  La respuesta puede variar ligeramente en redacción, ejemplos o estructura, pero mantiene coherencia conceptual. No suele cambiar el significado, solo la forma.

- Con `temperature = 1.5`:  
  La respuesta puede variar más notablemente entre ejecuciones. Cambia el estilo, los ejemplos y el orden de las ideas con mayor frecuencia.

---

### 2. ¿Cómo cambia el estilo y la creatividad de la respuesta?

- `temperature = 0`:  
  Estilo más técnico, directo y predecible. Baja creatividad. Ideal cuando se necesita consistencia.

- `temperature = 0.7`:  
  Equilibrio entre coherencia y variabilidad. El texto suena natural, con cierta flexibilidad en la redacción.

- `temperature = 1.5`:  
  Mayor creatividad y diversidad léxica. Puede introducir ejemplos más variados o estructuras menos comunes. También aumenta el riesgo de respuestas menos precisas.

---

### 3. ¿Qué valor usarías para cada caso?

- Asistente de atención al cliente:  
  Usaría `temperature = 0` o máximo `0.3`. Se prioriza precisión, consistencia y respuestas controladas.

- Generador de poesía:  
  Usaría `temperature = 1` o superior (por ejemplo 1.2 o 1.5). Se busca variabilidad, metáforas y creatividad.
...
Porque los tokens determinan el costo de uso del modelo y el límite máximo de contexto.  
Más tokens implican mayor costo y mayor uso de memoria de contexto. Además, si se alcanza el límite máximo, el modelo puede truncar información relevante.
...
- Incrementa el número de prompt tokens.
- Reduce el espacio disponible para la respuesta dentro del límite de contexto.
- Aumenta el costo total de la petición.
- Puede generar respuestas más lentas.

En escenarios productivos, enviar contexto innecesario impacta directamente en eficiencia y presupuesto.

---

## 3. ¿Cuál es la diferencia entre `temperature=0` y `temperature=1.5`?

La diferencia está en cómo se ajusta la distribución de probabilidad (softmax) antes de seleccionar el siguiente token:

- `temperature = 0`:  
  La distribución se vuelve más “picuda”, favoreciendo casi exclusivamente el token más probable. Resultado determinístico.

- `temperature = 1.5`:  
  La distribución se aplana, aumentando la probabilidad de tokens menos probables. Resultado más creativo y diverso, pero menos predecible.

En términos prácticos:  
`temperature=0` prioriza precisión y repetibilidad.  
`temperature=1.5` prioriza creatividad y variación.
...
> Nota: los siguientes números (tiempos/tokens) son **simulados** a modo de ejemplo, ya que dependen de red, región, versión del SDK, modelo exacto y carga del proveedor.

#### Salidas resumidas por modelo

- **OpenAI (gpt-4o-mini)**: explicación clara + ejemplo recursivo típico (factorial) con caso base explícito.
- **Gemini (gemini-1.5-flash / equivalente)**: muy directo, ejemplo corto, menos detalle teórico.
- **Claude (claude-3-5-haiku / equivalente)**: texto más pedagógico y estructurado, suele añadir advertencias (caso base / stack overflow).

#### Tabla de comparación (simulada)

| Métrica | OpenAI | Gemini | Claude |
|---------|--------|--------|--------|
| Tokens usados (total) | 560 | 510 | 620 |
| Tiempo de respuesta (s) | 1.25 | 0.95 | 1.55 |
| Longitud de respuesta (caracteres) | 980 | 860 | 1120 |
| Calidad de la explicación (1-10) | 9 | 8 | 9 |
| Calidad del código Python (1-10) | 9 | 8 | 9 |
| Calidad subjetiva general (1-10) | 9 | 8 | 9 |

#### (Opcional) Comparativa extra usando OpenRouter (modelos gratuitos)

| Métrica | Gemini 2.0 Flash (free) | DeepSeek R1 (free) | Llama 4 Scout (free) |
|---------|--------------------------|---------------------|----------------------|
| Tokens usados (total) | 520 | 900 | 650 |
| Tiempo de respuesta (s) | 1.05 | 1.80 | 1.30 |
| Calidad subjetiva general (1-10) | 8 | 9 | 8 |

### Respuestas (Preguntas de Reflexión)

1. **Mejor respuesta:** en este ejemplo, **Claude/OpenAI** empatan por claridad, estructura y buen código; DeepSeek suele ser muy fuerte cuando se necesita razonamiento paso a paso.
2. **Más rápido:** normalmente un modelo “flash” tipo **Gemini Flash**. La velocidad importa mucho en productos interactivos (chat/UX), menos en tareas batch nocturnas.
3. **Cuándo elegir cada uno:**
    - **OpenAI:** buena calidad general + tooling/ecosistema.
    - **Gemini:** velocidad/coste y buen rendimiento en tareas generales.
    - **Claude:** estilo más “redaccional” y explicaciones cuidadas.
4. **Estructura:** sí; Gemini tiende a ser más breve, Claude más didáctico, OpenAI suele equilibrar explicación + ejemplo.
...
Reglas:
- Responde en español.
- Explica en pasos cortos y con ejemplos.
- Si te piden "lo anterior", asume que se refieren al último concepto explicado.
...
def chat():
    ...
    while True:
        ...
        if getattr(response, "usage", None):
            ...
            print("  [Tokens: no disponible en este proveedor/modelo]")
...
### Prueba de memoria (simulada / ejemplo)

> Nota: esta transcripción es **simulada** para ilustrar el comportamiento esperado.

1) Usuario: ¿Qué son las variables en Python?

Tutor (resumen): Una variable es un “nombre” que apunta a un valor (número, texto, etc.). Se asigna con `=` y Python infiere el tipo. Ejemplo: `edad = 20`.

2) Usuario: Dame un ejemplo de lo anterior

Tutor (resumen): Claro: `nombre = "Ana"` y luego `print(nombre)`. Aquí “lo anterior” se refiere a **variables** y a la **asignación**.

3) Usuario: Ahora muéstrame cómo usar listas

Tutor (resumen): Una lista guarda varios valores: `nums = [1, 2, 3]`. Puedes acceder con índices `nums[0]` y añadir con `append`.

4) Usuario: ¿Cuál es la diferencia entre lo primero que me explicaste y esto?

Tutor (resumen): Las variables son nombres para **un** valor (aunque ese valor puede ser una lista). Las listas son un **tipo de dato** que agrupa varios elementos. Ej: `x = 5` vs `x = [5, 6]`.

### Respuestas (Preguntas de Reflexión)

1. No mantienen estado porque cada request HTTP debe ser autocontenida; el proveedor no “recuerda” tu conversación salvo que tú envíes el historial.
2. **Ventajas:** menor coste/tokens, menor latencia, menos riesgo de “deriva” por contexto antiguo. **Desventajas:** el bot olvida detalles y puede perder coherencia.
3. Usaría una mezcla de: (a) resumen incremental del historial, (b) memoria persistente (DB) por usuario, (c) recuperación por embeddings (RAG) para traer solo lo relevante.
...
### Respuestas (simuladas / ejemplo)

1. **Intentos:**
    - Oferta de empleo: 1 intento.
    - Reseña de producto: 1 intento.
    - Noticia: 1 intento.

2. **"No especificado":** sí, típicamente en **empresa** (en el texto no aparece el nombre explícito). Es correcto. (Algunos modelos podrían inferir “TechCorp” por el dominio del email, pero es una inferencia.)
3. **Numéricos:** deberían salir como números (`45000`, `55000`, `1299`, `7`, `10`, `30`, `2021`), no como strings. Si salen como strings, se corrige con reintento y recordatorio en el prompt.
4. **Otro idioma:** si el modelo entiende ese idioma, extraerá igual; si no, aumentan errores y “No especificado”. En producción, conviene detectar idioma y/o traducir antes de extraer.
...
|---------|--------------------|--------------------|
| Líneas de código (aprox.) | 70-90 | 35-55 |
| Facilidad de lectura | Media (más “boilerplate”) | Alta (pipeline declarativo) |
| Gestión de reintentos | Manual (tú lo controlas) | Requiere wrapper adicional |
| Cambiar de modelo | Fácil, pero hay que tocar más sitios | Muy fácil (cambiar `ChatOpenAI(...)`) |
| Curva de aprendizaje | Baja (solo SDK) | Media (conceptos de LangChain) |
...
1. Aproximadamente **30-40 líneas** (depende de si incluyes validación/reintentos en ambos).
2. Separa responsabilidades: prompt (entrada) → modelo (razonamiento) → parser (salida). Es más testeable y fácil de recombinar.
3. No lo usaría cuando: (a) el proyecto es pequeño y el SDK nativo basta, (b) necesito control fino de reintentos/errores/telemetría, (c) quiero minimizar dependencias.
4. Usaría `ChatAnthropic` (paquete `langchain-anthropic`) y cambiaría el modelo; el resto del pipeline (`prompt | model | parser`) se mantiene.
...
### Implementación (realizada en este workspace)

- `ejercicios/tema6/llm_client.py`: implementación completa de `LLMClient` para `openai`, `openrouter`, `gemini`, `claude`.
- `ejercicios/tema6/test_client.py`: script de prueba que **omite** proveedores sin API key en el `.env`.
